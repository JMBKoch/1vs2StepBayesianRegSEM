...
resultsSVNP <- resultsSVNP%>% 
  mutate(sigmaSqu = sigma^2)

resultsSVNP_hyper <- 
resultsSVNP_hyper %>% 
  mutate(sigmaSqu = -9999)

resultsSVNP_comb <- bind_rows(resultsSVNP, resultsSVNP_hyper)
```
...
resultsSVNP_comb <- 
  resultsSVNP_comb %>% 
  arrange(sigmaSqu, N, cross, iteration) %>% 
  mutate(
...
PowerSVNP %>% 
  ...
  select(N, sigmaSqu, cross, criterion, value) %>% 
    mutate(
    sigmaSqu = ifelse(
      sigmaSqu == -9999.000, "\u03c3 ~ Cauchy(0, 1)", 
       paste0("\u03c3\u00B2 = ", sigmaSqu)
      ),
    sigmaSqu = as.factor(sigmaSqu),
...
TypeISVNP %>% 
  ...
  mutate( criterion = as.factor(str_replace_all(Criterion, c("typeITres0.15Mean_2" = "Treshold 0.15", 
           ...
           criterion =  factor(criterion, levels = c("50% Credible Interval", "80% Credible Interval", "90% Credible Interval",  "95% Credible Interval", "Treshold 0.15", "Treshold 0.10", "Treshold 0.05", "Treshold 0"))) %>% 
  select(N, sigmaSqu, cross, criterion, value) %>%   mutate(
    sigmaSqu = ifelse(
      sigmaSqu == -9999.000, "\u03c3 ~ Cauchy(0, 1)", 
       paste0("\u03c3\u00B2 = ", sigmaSqu)
      ),
...
```{r}
# define color, shape and lines
# Define a reusable list of scales
custom_scales <- list(
  scale_linetype_manual(
    "criterion", 
    values = c("solid", "solid", "solid", "solid", "twodash", "twodash", "twodash", "twodash")
  ),
  scale_color_manual(
    "criterion", 
    values = c(
      # First four: Reds from least to most saturated
      "#ffcccc", "#ff9999", "#ff6666", "#ff0000",
      # Last four: Blues from least to most saturated
      "#ccccff", "#9999ff", "#6666ff", "#0000ff"
    )
  ),
  scale_shape_manual(
    "criterion", 
    values = c(
      16, 16, 16, 16,  # Points for the first four
      17, 17, 17, 17   # Triangles for the second four
    )
  ),
    # Adjust size for shapes to be smaller, similar to shape = 4 default
  theme(
    legend.key.size = unit(0.6, "cm"),  # Adjust legend key size (optional, to match the smaller shapes)
...
Power <- 
PowerLong %>% 
  ggplot(mapping = aes(x = as.factor(cross), y = value, group= criterion, col = criterion, linetype = criterion, shape = criterion))+
  geom_line(size = 0.1)+
  geom_point()+
...
```
```{r}
# plot juist for N = 100 for final pub
Power_100 <- 
PowerLong %>% 
  filter(N == "N = 100") %>% 
  ggplot(mapping = aes(x = as.factor(cross), y = value, group= criterion, col = criterion, linetype = criterion, shape = criterion))+
...
Power_100+
  labs(title = "Power SVNP",
       x = NULL)  
readr::write_rds(TypeISVNP_100, file = "~/1vs2StepBayesianRegSEM/Rmd/figures/typeISVNP.Rds")
readr::write_rds(PowerSVNP_100, file = "~/1vs2StepBayesianRegSEM/Rmd/figures/powerSVNP.Rds")
```
