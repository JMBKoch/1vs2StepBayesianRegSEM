...
These indices don't seem to belong to a certain conditionset. We can simply find the set of conditions belong to this by subsetting the first row of the results object.

```{r}
resultsSVNP_hyper %>% 
  select(N, cross, iteration)
```
...
On average, they appear to be quite low. There appears to be no set of conditions where the proportion of divergent transition is lager than 0.005. 
...
```{r}
# match conditions on run-indices
pos
```
...
By this you would lose 0 rows, which does not seem so bad.
...
nrow(resultsSVNP_hyperFiltered) + length(indicesCombined)
# Save filtered data as .Rds file for further analysis
saveRDS(resultsSVNP_hyperFiltered, file = "~/1vs2StepBayesianRegSEM/output/resultsSVNP_hyperFilter.Rds")
```

```{r}
nrow(resultsSVNP_hyperFiltered)
```
