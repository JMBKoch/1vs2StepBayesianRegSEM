...
SEM). A notable exception is the work by (\textbf{zhang2023bayesian?}) who
...
\(s_{slab}^2\) (Piironen \& Vehtari, 2017b). (\textbf{zhang2023bayesian?}) showed that
...
We included a configuration of the SVNP where a half-Cauchy prior centered around 0 and with a scale of 1 was set as hyperprior for the standard-deviation \(\sigma\) (not the variance \(\sigma^2\))\footnote{Note that a hyper-prior was set for \(\sigma\) and not \(\sigma^2\) as this is the way to specify a normal distribution Stan. We chose to refer to \(\sigma^2\) in the general introduction of this shrinkage prior as it aligns more closely with the classical introduction of this approach by Muthén and Asparouhov (2012).}. We deemed this relatively simple specification to be suitable for assessing the benefits of a hyperprior over a fixed variance, given the simple nature of the model studied.
...
(\textbf{zhang2023bayesian?}) found the RHSP to perform well in selecting
...
SEM models. For instance, extending the work by (\textbf{zhang2023bayesian?}) who
...
(\textbf{van2023bayesian?}) emphasizes the need for user-friendly software for
...
Muthén, \& Muthén, B. O. (1998-2017). \emph{Mplus {User}'s {Guide}}. Retrieved from \url{https://www.statmodel.com/html_ug.shtml}
