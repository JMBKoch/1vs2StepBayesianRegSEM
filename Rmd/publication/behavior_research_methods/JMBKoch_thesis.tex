...
\hypertarget{the-svnp-with-a-hyper-prior}{%
\subsubsection{The SVNP with a hyper-prior}\label{the-svnp-with-a-hyper-prior}}

The classical Bayesian CFA approach by Muthén and Asparouhov (2012) asks for a fixed variance parameter. However, it is common in regularization to set hyper-priors on the parameters of priors, in order to allow for more flexibility in model estimation (Van Erp et al., 2019) Such approach can lead to more sparsimonious models, as it allows for a more flexible model estimation, where not all parameters need to be shrunken in the same (heavy) amount. Software packages for Bayesian CFA such as Mplus (L. K. Muthén \& Muthén, 1998) and BLAVAAN (Merkle, Fitzsimmons, Uanhoro, \& Goodrich, 2020) have for the longest time not allowed to set hyper-priors for the variance of the SVNP. Only in probabilistic Bayesian modeling software such as \texttt{Stan} this possibility has been available (Stan Development Team, 2021; Van Erp, 2023). Since such software packages may not be a practical choice for applied researchers in terms of user-friendliness, the fixed-variance approaches has remained a relevant go-to approach (Van Erp, 2023). Recently, the BLAVAAN package has been updated to allow setting hyper-priors for the variance parameter of the SVNP. (\emph{Als ik het goed begreep kan je daar dus nu wel een hyper-prior bepalen? REF naar blavaan versie release notes o.i.d? Ik probeeder i.i.g. om duidelijk te maken dat de fixed-variance approach door z'n availability vergeleken met de hyper-prior approach nog steeds harstikke relevant is om te onderzoeken.}).

\emph{Hier ben ik niet zeker wat handig is om op te schrijven\ldots Lijkt me een goed idee om dit meer stapsgewijs op te bouwen}
...
\hypertarget{svnp-with-hyperior}{%
\subsubsection{SVNP with Hyperior}\label{svnp-with-hyperior}}

We included a configuration of the SVNP where a half-Cauchy prior centered around 0 and with a scale of 1 was set as hyper-prior for the standard-deviation \(\sigma\) (not the variance \(\sigma^2\))\footnote{Note that a hyper-prior was set for \(\sigma\) and not \(\sigma^2\) as this is the way to specify a normal distribution Stan. We chose to refer to \(\sigma^2\) in the general introduction of this shrinkage prior as it aligns more closely with the classical introduction of this approach by Muthén and Asparouhov (2012).}. We deemed this relatively simple specification to be suitable for assessing the benefits of a hyper-prior over a fixed variance, given the simple nature of the model studied.
...
(e.g., by adding it to the \texttt{BLAVAAN} package, Merkle et al., 2020).
...
Muthén, L. K., \& Muthén, B. O. (1998). \emph{Mplus {User}'s {Guide}}. Retrieved from \url{https://www.statmodel.com/html_ug.shtml}
