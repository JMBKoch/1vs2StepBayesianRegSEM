...
\begin{center}
TITLE: ”Getting a Step Ahead: Using the Regularized Horseshoe
Prior to Select Cross-Loadings in Bayesian CFA”
\end{center}
...
\emph{shrinkage prior}. This prior attaches large
...
regression (Piironen \& Vehtari, 2017b; Van Erp, Oberski, \& Mulder, 2018). However,
...
SEM). A notable exception is the work by Zhang and Liang (2024) who
...
illustrated exist (Jacobucci \& Grimm, 2018; Van Erp, 2023). However,
...
and model simplicity is \emph{regularization} (Hastie, Tibshirani, \& Wainwright, 2015).
...
inference (Jacobucci \& Grimm, 2018; Jacobucci et al., 2016).
Bayesian approaches to regularization overcome these obstacles
(Jacobucci \& Grimm, 2018). In Bayesian model estimation, the
...
Laplace- (double exponential) prior for parameters (Hans, 2009; Park \& Casella, 2008; see Van Erp et al., 2018 for the Bayesian
...
be implemented in \texttt{Stan}, one of the most popular software packages for
MCMC-sampling, as \texttt{Stan} does not allow for discrete mixture priors
...
implementable in \texttt{Stan} is the so-called \emph{Regularized Horseshoe Prior}
...
parameters substantially to zero in practice (Carvalho et al., 2010; Datta \& Ghosh, 2013; Polson \& Scott, 2010; Van Der Pas, Kleijn, \& Van Der Vaart, 2014). However, due to its
...
\(s_{slab}^2\) (Piironen \& Vehtari, 2017b). Zhang and Liang (2024) showed that
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{JMBKoch_thesis_files/figure-latex/unnamed-chunk-1-1.pdf}}
\caption{\label{fig:unnamed-chunk-1}Density Plots of the Shrinkage Priors of Interest. A shiny app where the priors can be plotted for all hyper-parameters settings in this study can be found on \url{https://jmb-koch.shinyapps.io/shiny/}.}
\end{figure}
...
\texttt{cmdstandr} to interface \texttt{Stan} with R (Gabry \& Češnovar, 2022), while also
...
(Wickham \& Girlich, 2022) and \texttt{dplyr} (Wickham, François, \& Müller, 2022). All plots were
...
2).\footnote{The \texttt{Stan} code of the model can be found on
...
\texttt{Stan}'s default uniform prior.
...
We included a configuration of the SVNP where a half-Cauchy prior centered around 0 and with a scale of 1 was set as hyper-prior for the standard-deviation \(\sigma\) (not the variance \(\sigma^2\))\footnote{Note that a hyper-prior was set for \(\sigma\) and not \(\sigma^2\) as this is the way to specify a normal distribution in \texttt{Stan}. We chose to refer to \(\sigma^2\) in the general introduction of this shrinkage prior as it aligns more closely with the classical introduction of this approach by Muthén and Asparouhov (2012).}. We deemed this relatively simple specification to be suitable for assessing the benefits of a hyper-prior over a fixed variance, given the simple nature of the model studied.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{~/1vs2StepBayesianRegSEM/Rmd/figures/model.png}}
...
\bf{SVNP}: $\sigma \sim half-Cauchy(0, 1)$ &  & \\
location & 1 & 0\\
scale & 1 & 1\\
\bf{RHSP} &  & \\
$s_{global}$ & 2 & 0.1, 1\\
...
\subsubsection{SVNP with hyper-prior}\label{svnp-with-hyper-prior}
...
\begin{verbatim}
## [1] "/Users/michi/1vs2StepBayesianRegSEM/Rmd/publication/behavior_research_methods"
\end{verbatim}
...
substantially between different hyper-parameter configurations\footnote{See \url{https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/RHSP} for the results of the RHSP with different hyper-parameter settings.}. The
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{JMBKoch_thesis_files/figure-latex/unnamed-chunk-6-1.pdf}}
\caption{\label{fig:unnamed-chunk-6}Mean Power and Type-I-Error Rates in Selecting non-zero Cross Loadings (N = 100). All hyper-parameters of the RHSP are set to 1 in the results presented here. The red dashed horizontal lines represents a Power level of 0.80.}
\end{figure}
...
one, and we focus on the results for N = 100\footnote{The results for N = 200 of the RSHP, and for the other hyperparmatersettings, can be found on \url{https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/RHSP}. The results for the SVNP with n = 200 can be found on \url{https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/SVNP}. The results for the SVNP with hyper-prior and n = 200 can be found on \url{https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/SVNP_hyper}.}. In the plots summarizing the Power the horizontal red dashed line
...
Zhang and Liang (2024) found the RHSP to perform well in selecting
...
SEM models. For instance, extending the work by Zhang and Liang (2024) who
...
\subsection{Conflicts of interest/Competing interests}\label{conflicts-of-interestcompeting-interests}
...
\subsection{Availability of data and materials (data transparency)}\label{availability-of-data-and-materials-data-transparency}

Data and materials are available at the github repository that is cited throughout the text: \url{https://github.com/JMBKoch/1vs2StepBayesianRegSEM/}.

\subsection{Code availability (software application or custom code)}\label{code-availability-software-application-or-custom-code}

Data and materials are available at the github repository that is cited throughout the
text: \url{https://github.com/JMBKoch/1vs2StepBayesianRegSEM/}
...
\bibitem[\citeproctext]{ref-aust_papaja_2022}
...
\bibitem[\citeproctext]{ref-betancourt_conceptual_2018}
...
\bibitem[\citeproctext]{ref-carvalho_horseshoe_2010}
...
\bibitem[\citeproctext]{ref-cham_estimating_2012}
...
\bibitem[\citeproctext]{ref-chen_jinsong_partially_2021}
...
\bibitem[\citeproctext]{ref-cox_principles_2006}
Cox, D. R. (2006). \emph{Principles of {Statistical} {Inference}}. Cambridge University Press.

\bibitem[\citeproctext]{ref-datta_asymptotic_2013}
...
\bibitem[\citeproctext]{ref-ghosh_use_2018}
...
\bibitem[\citeproctext]{ref-hans_bayesian_2009}
...
\bibitem[\citeproctext]{ref-hastie_statistical_2015}
...
\bibitem[\citeproctext]{ref-hoerl_ridge_2000}
...
\bibitem[\citeproctext]{ref-hsiang_bayesian_1975}
...
\bibitem[\citeproctext]{ref-huang2020lslx}
Huang, P.-H. (2020). Lslx: Semi-confirmatory structural equation modeling via penalized likelihood. \emph{Journal of Statistical Software}, \emph{93}, 1--37.

\bibitem[\citeproctext]{ref-ishwaran_spike_2005}
...
\bibitem[\citeproctext]{ref-jacobucci_practical_2019}
...
\bibitem[\citeproctext]{ref-jacobucci_comparison_2018}
...
\bibitem[\citeproctext]{ref-jacobucci_regularized_2016}
...
\bibitem[\citeproctext]{ref-maccallum_model_1992}
...
\bibitem[\citeproctext]{ref-merkle_efficient_2020}
...
\bibitem[\citeproctext]{ref-mitchell_bayesian_1988}
...
\bibitem[\citeproctext]{ref-muthen_bayesian_2012}
Muthén, \& Asparouhov, T. (2012). \emph{Bayesian {SEM}: {A} more ﬂexible representation of substantive theory}. 78. \url{https://doi.org/10.1037/a0026802}

\bibitem[\citeproctext]{ref-park_bayesian_2008}
...
\bibitem[\citeproctext]{ref-piironen_hyperprior_2017}
...
\bibitem[\citeproctext]{ref-polson_shrink_2010}
...
\bibitem[\citeproctext]{ref-serang_exploratory_2020}
...
\bibitem[\citeproctext]{ref-serang_exploratory_2017}
...
\bibitem[\citeproctext]{ref-tibshirani_regression_2011}
...
\bibitem[\citeproctext]{ref-van_der_pas_horseshoe_2014}
...
\bibitem[\citeproctext]{ref-van_erp_bayesian_2023}
Van Erp, S. (2023). Bayesian {Regularized} {SEM}: {Current} {Capabilities} and {Constraints}. \emph{Psych}, \emph{5}(3), 814--835. \url{https://doi.org/10.3390/psych5030054}

\bibitem[\citeproctext]{ref-van_erp_shrinkage_2018}
Van Erp, S., Oberski, D. L., \& Mulder, J. (2018). Shrinkage priors for {Bayesian} penalized regression. \emph{Journal of Mathematical Psychology}, \emph{89}, 31--50. \url{https://doi.org/10.1016/j.jmp.2018.12.004}
...
\bibitem[\citeproctext]{ref-wickham_dplyr_2022}
...
\bibitem[\citeproctext]{ref-wickham_tidyr_2022}
...
\bibitem[\citeproctext]{ref-yamaguchi2023fully}
...
\bibitem[\citeproctext]{ref-zhang_bayesian_2024}
Zhang, L., \& Liang, X. (2024). Bayesian regularization in multiple-indicators multiple-causes models. \emph{Psychological Methods}, \emph{29}(4), 679--703. \url{https://doi.org/10.1037/met0000594}

\bibitem[\citeproctext]{ref-zhang_criteria_2021}
