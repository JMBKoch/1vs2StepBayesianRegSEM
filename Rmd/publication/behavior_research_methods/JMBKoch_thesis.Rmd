...
model of this study with little bias, only the two priors with a hyper-prior were able to do so consistently. This illustrate the fundamental of using a hyper-prior approach over using fixed parameters. The RHSP did not outperform the SVNP with a hyper-prior and showed substantially poorer convergence. Hence, for this particular model the SVNP with hyperprior struk the most ideal balance in model simplicity and model complexity, by yielding most accurate solutions with little estimation complexity. In more complex models the refined nature of the RHSP with its several hyperpriors may ultimately prove advantegous even over the SVNP with hyper-prior, as it allows for even more flexibility. Future research may compare the SVNP, also with more refined hyper-prior configurations, to the RHSP in regularization of more complex SEM models. The current results do suggest, however, that for simple CFA model, using a RHSP is an overkill solution that may lead to more problems in terms of actually being able to estimate the model than benefits in terms of accurate shrinkage.  
...
approach for applied researchers. Next to this, the RHSP may thus work more reliably with larger sample sizes. In sum, there remain thus plenty of potential use-cases for the RHSP over the SVNP with hyper-prior in regularizing more complex models on larger datasets.

Under simple models such as the one employed in this study
using the least complex version of SVNP, with the variance fixed to a relatively large value of $\sigma^2$ may also be advisable. This approach performed as well as both hyper-prior approaches. It also remains the most accessible appraoch, due to it's simpleness and availabilty across Bayesian CFA software   However, simply using larger values of $\sigma^2$ with the SVNP is
...
to over-fitting. Because of this, the SVNP with hyperprior or the RHSP might prove as more advisable for more complex models. In particular the RHSP with its refined nature in giving prior mass only where strictly
...
However, future research comparing the SVNP both with and without, and for several hyper-prior setting, to the RHSP in regularizing
