...
*shrinkage prior* [@hastie_statistical_2015]. This prior attaches large
...
In practice the hyper-parameter $\lambda$ is often determined by means of
...
and $\lambda$ is again a hyper-parameter that determines the amount of
...
TIn classical penalization, the optimal penalty parameter $\lambda$ (not to be confused with the $\lambda$'s representing factor loadings in CFA) is determined via cross-validation. In the Bayesian framework, an alternative approach is possible in which a hyper-prior is specified for the variance parameter in the SVNP. As a result, the optimal value for the variance, which determines the amount of shrinkage similar to the classical penalty parameter, is estimated simultaneously with the model. This removes the arbitrary and choice of setting the prior variance in the SVNP approach. Moreover, when such a hyper-prior is specified, the ridge prior shows more desirable shrinkage behavior compared to a setting in which the variance is fixed [see Figures 5 and 6 in @van_erp_shrinkage_2018]. An often used choice for the hyper-prior is a half-Cauchy, leading to the following specification for a cross-loading $\lambda_{c,jk}$:
...
[@wickham_tidyr_2022] & `dplyr` [@wickham_dplyr_2022]. All plots were
...
```{r, dev='cairo_pdf', include = T, warning = F, fig.cap='Mean Power and Type-I-Error Rates in Selecting non-zero Crossloadings (N = 100). All hyper-parameters of the RHSP are set to 1 in the results presented here. The red dashed horizontal lines represents a power level of 0.80.', fig.height=10, fig.width=8}
library(patchwork)
library(ggplot2)
...
to over-fitting. Because of this, and based on our results, we advise against the use of SVNP with fixed variance and instead recommend a SVNP with a hyperprior or the RHSP. In particular the RHSP with its refined nature in giving prior mass only where strictly desired (see Figure 1) might allow for a more efficient identification
of complex models than the SVNP, at least for some hyper-parameter
settings. Using the RHSP may thus turn out to be the approach of choice
in situations where identification is hard to achieve with the SVNP. Future research comparing the SVNP with fixed variances and with a variety of hyper-prior setting to the RHSP in regularizing
...
[@zhang_criteria_2021], or projection predictive inference
...
The large levels of bias found in for the majority of fixed-variance configurations of the SVNP are not surprising, given the original approach
