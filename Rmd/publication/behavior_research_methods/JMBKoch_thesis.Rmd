...
regularizing cross loadings in Bayesian Confirmatory Factor Analysis (CFA). The SVNP is routinely used to
...
Moreover, the RHSP showed more robust results across sample sizes, and different
...
the low sample sizes considered. Implications for the use of Bayesian regularization in CFA are discussed.

\vfill

# Acknowledgements

This research was supported by a NWO Veni Grant (Vl.Veni.221G.005) to Sara Van Erp.

# Keywords

*Regularization*, *Bayesian Regularized SEM*, *Small Variance Normal
Prior*, *Regularized Horseshoe Prior*.
...
complexity arises, that is mostly known as *bias-variance trade-off*.
...
required to estimate them [@cox_principles_2006;
@james_introduction_2021].

Confirmatory factor analysis [CFA, see e.g. @bollen_structural_1989] is
a commonly used statistical technique to model unobservable
...
an indispensable tool in social science research. In CFA it is common
...
[@maccallum_model_1992].

As a Bayesian solution to this issue @muthen_bayesian_2012 proposed
identifying CFA models by setting the so-called *Small Variance Normal
Prior* (SVNP) for cross loadings, which is a normal distribution with
mean zero and a very small variance (e.g., $\sigma^2$ = 0.01). As a result, 
the simple structure assumption of exact zero cross-loadings is relaxed 
to allow approximately zero cross-loadings in the model. This is
an instance of *regularization*, with the SVNP being used as the
*shrinkage prior*. This prior attaches large
...
undesired levels of bias to the model [@lu_bayesian_2016]. Consequently,
...
[@muthen_bayesian_2012]. The model is then re-estimated, where cross
...
researchers [@zhang_criteria_2021]. Additionally, in this two-step
...
regression [@piironen_sparsity_2017; @van_erp_shrinkage_2018]. However,
systematic research on the performance of this shrinkage prior in
Bayesian regularized SEM is lacking [see @van_erp_bayesian_2023 for an
overview on the shrinkage priors previously used in Bayesian Regularized
SEM]. A notable exception is the work by @zhang_bayesian_2024 who
investigated the horseshoe prior in a multiple indicator multiple causes
(MIMIC) model [see also @yamaguchi2023fully for work on the Horseshoe
...
illustrated exist [@van_erp_bayesian_2023; @jacobucci_comparison_2018]. However,
...
In the remainder of this article we will first introduce regularization and 
discuss it from both a Bayesian and frequentist perspective. We will
...
performance of the RHSP versus the SVNP in selecting cross loadings in a
...
and model simplicity is *regularization* [@hastie_statistical_2015].
...
predictors, are shrunken to (or towards) zero. Consider a regression model,
...
Penalized regression adds a penalty term to this cost function, which
...
[@tibshirani_regression_1996; @tibshirani_regression_2011]. Here, the
...
In practice the penalty-parameter $\lambda$ is often determined by means of
cross-validation. When L = 2 (the so-called L2 norm),
$||\beta||_2 = \sqrt{\Sigma_{j=1}^p \beta_j^2}$. This is the famous
ridge penalty [@hoerl_ridge_2000]. Here, the same general principle is
...
former has the ability to shrink coefficients entirely to zero (thereby actively
selecting predictors), whereas in ridge regression coefficients are only
shrunken approximately to zero.
...
and $\lambda$ is again a penalty-parameter that determines the amount of
...
[@jacobucci_regularized_2016]. Also structural model parameters, such as
regression coefficients in MIMIC models [@jacobucci_regularized_2016;
@jacobucci_practical_2019], or indirect effects in mediation models with
continuous [@serang_exploratory_2017] or dichotomous outcomes
[@serang_exploratory_2020], have successfully been regularized through
the usage of penalties such as the LASSO or ridge penalty. R-packages
such as `lslx`  [@huang2020lslx] or `regsem` 
[@li2021tutorial] have implemented classical penalties in various SEMs.
...
inference [@jacobucci_regularized_2016; @jacobucci_comparison_2018].
Bayesian approaches to regularization overcome these obstacles
[@jacobucci_comparison_2018]. In Bayesian model estimation, the
...
[@hsiang_bayesian_1975]. The LASSO penalty can be mimicked by setting a
Laplace- (double exponential) prior for parameters [@park_bayesian_2008;
@hans_bayesian_2009; see @van_erp_shrinkage_2018 for the Bayesian
...
a central advantage. This renders it unnecessary to analytically derive 
standard errors, as the variance of the posterior distributions of
(functions of) model parameters are directly available. Also complex shrinkage priors with
...
Confirmatory Factor Analysis [CFA, @bollen_structural_1989] is an
...
loads on which items, is assumed. This makes it possible to formulate clear
...
parameter (for instance, a cross loading) to or from the model. Based on
...
over-fitting may occur [@maccallum_model_1992].

As solution to this issue, @muthen_bayesian_2012, proposed a Bayesian
...
[@muthen_bayesian_2012]. First, the model is estimated with the
...
### The SVNP with hyper-prior 

In classical penalization, the optimal hyper parameter $\lambda$ (not to be confused with the $\lambda$'s representing factor loadings in CFA) is determined via cross-validation. In the Bayesian framework, an alternative approach is possible in which a hyper-prior is specified for the variance parameter in the SVNP. As a result, the optimal value for the variance, which determines the amount of shrinkage similar to the classical penalty parameter, is estimated simultaneously with the model. This removes the arbitrary choice of setting the prior variance in the SVNP approach. Moreover, when such a hyper-prior is specified, the ridge prior shows more desirable shrinkage behavior compared to a setting in which the variance is fixed [see Figures 5 and 6 in @van_erp_shrinkage_2018]. An often used choice for the hyper-prior is a half-Cauchy, leading to the following specification for a cross-loading $\lambda_{c,jk}$:
$$\lambda_{c,jk}  \sim \mathcal{N}(0, \sigma^2), \ where$$
...
successfully been applied in regularized SEM previously [e.g.
@chen_jinsong_partially_2021; @zhang_criteria_2021]. In general, its
...
posterior means, will never be entirely zero [@zhang_criteria_2021]. As
...
@zhang_criteria_2021 pointed out that it is difficult to formulate
...
Spike-and-Slab Prior [@george_variable_1993; @ishwaran_spike_2005;
@mitchell_bayesian_1988]. This prior is a discrete mixture of an
...
et al. (2016) found that the Spike-and-Slab Prior performs well in shrinking
...
be implemented in `Stan`, one of the most popular software packages for
MCMC-sampling, as `Stan` does not allow for discrete mixture priors
[@stan_development_team_stan_2021; @betancourt_conceptual_2018]. This
calls for a *non-discrete* alternative shrinkage prior that also
...
implementable in `Stan` is the so-called *Regularized Horseshoe Prior*
[RHSP, @piironen_hyperprior_2017; @piironen_sparsity_2017]. This prior
is an extension of the Horseshoe Prior [@carvalho_horseshoe_2010]. The
main idea of the original Horseshoe Prior is that there is a *global
shrinkage parameter* $\tau$, shrinking all cross loadings to zero. Next
to this, there is a *local shrinkage parameter* $\bar{\omega}_{jk}$[^1]
...
[@polson_shrink_2010]. Formally, the Horseshoe prior for every cross
...
individual cross loadings [@carvalho_horseshoe_2010;
@piironen_sparsity_2017]. This shrinkage factor ranges from zero to one,
...
[^1]: We deviate from the common notation of the local shrinkage
...
parameters substantially to zero in practice [@carvalho_horseshoe_2010;
@polson_shrink_2010; @datta_asymptotic_2013;
@van_der_pas_horseshoe_2014]. However, due to its
...
[@ghosh_use_2018]. The RHSP prevents such issues by shrinking also large
...
much lower than 1 [@piironen_sparsity_2017]. Note that we specify the
...
speaking, the prior is only a Regularized *Horseshoe* Prior when this is
the case. In the current study we vary the degrees of freedoms of all
scale parameters to assess the extent to which the sparsifying
...
$s_{slab}^2$ [@piironen_sparsity_2017]. @zhang_bayesian_2024 showed that
...
```{r, cache=T, dev='cairo_pdf', include=T, echo=F, warning=F, fig.cap="Density Plots of the Shrinkage Priors of Interest. A shiny app where the priors can be plotted for all hyper-parameters settings in this study can be found on <https://jmb-koch.shinyapps.io/shiny/>."}
# Make Figure 1 with 
...
[@stan_development_team_stan_2021] and R [@r_core_team_r_2021]. We used
`cmdstandr` to interface `Stan` with R [@gabry_cmdstanr_2022], while also
heavily relying on `rstan` [@stan_development_team_rstan_2022] and
`bayesplot` [@gabry_bayesplot_2022] for the post-processing of the
posterior samples. Results were further post-processed using `tidyr`
[@wickham_tidyr_2022] and `dplyr` [@wickham_dplyr_2022]. All plots were
made using `ggplot2` [@wickham_ggplot2_2016]. We ran up to 46
replications in parallel using the `parallel` package
[@r_core_team_package_2022]. This manuscript was written using R
Markdown and the `papaja` package [@aust_papaja_2022]. All code that was
used to run the simulations can be openly accessed on the author's
[**github**](https://github.com/JMBKoch/1vs2StepBayesianRegSEM)[^2]. The
models were sampled using the No-U-Turn-Sampler [@homan_no-u-turn_2014],
...
[^2]: Specifically, the R-scripts needed to run the simulation can be
    found on
    <https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/R>.
    `parameters.R` can be adjusted to adjust study parameters, and
    `main.R` is used to run the main simulation. Required packages are
    listed at the top of `parameters.R`.
...
2).[^3] The factors were scaled by fixing their means to zero and their
...
[^3]: The `Stan` code of the model can be found on
    <https://github.com/JMBKoch/1vs2StepBayesianRegSEM/blob/main/stan/>.
...
0.01 and 0.1, based on @muthen_bayesian_2012. For the SVNP this left us
with a total number of 2 (size cross loading) x 2 (N) x 3
(hyper-parameter $\sigma^2$) = 12 individual conditions. Per condition,
200 replications were run, yielding a total of 2400 replications for
this prior. 

We included a configuration of the SVNP where a half-Cauchy prior centered around 0 and with a scale of 1 was set as hyper-prior for the standard-deviation $\sigma$ (not the variance $\sigma^2$)[^4]. We deemed this relatively simple specification to be suitable for assessing the benefits of a hyper-prior over a fixed variance, given the simple nature of the model studied. 

![Graphical Representation of the True
Model.](~/1vs2StepBayesianRegSEM/Rmd/figures/model.png)

[^4]: Note that a hyper-prior was set for $\sigma$ and not $\sigma^2$ as this is the way to specify a normal distribution in `Stan`. We chose to refer to $\sigma^2$ in the general introduction of this shrinkage prior as it aligns more closely with the classical introduction of this approach by @muthen_bayesian_2012.
...
$p_0$ [@piironen_sparsity_2017], and 0.1 a logical minimum given the
...
freedom of these two parameters, $df_{local}$ and $df_{global}$ were
varied between 1 and 3. Larger degrees of freedom may help to overcome
...
Conditions<- c("$\\bf{Population}$", "Sample size (N)", "Size cross loading $\\lambda_{c1 , 6}$", "\\bf{SVNP}", "$\\sigma^2$", "\\bf{SVNP}: $\\sigma \\sim half-Cauchy(0, 1)$", "location", "scale", "\\bf{RHSP}", "$s_{global}$", "$df_{global}$", "$s_{local}$", "$df_{local}$", "$s_{slab}$", " $df_{slab}$")
Levels <- c("","2", "2", "", "3", "", "1", "1", "" , "2", "2", "2", "2", "3", "2")
Values <- c("", "100, 200",  "0.2, 0.5", "", "0.1, 0.01, 0.001", "", "0"," 1", "", "0.1, 1", "1, 3", "0.1, 1", "1, 3", "0.1, 1, 5", "1, 3")
...
[^5]: see
    <https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/medianEstimates>
...
of results do not differ significantly for the MSE[^6].

[^6]: see
    <https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses>
...
[@zhang_criteria_2021]. First, we used a variety of thresholding rules,
...
[see @betancourt_conceptual_2018 for details].
...
### SVNP with hyper-prior

With the half-Cauchy hyper-prior, the SVNP showed somewhat weaker but still good convergence. Ten replications yielded no posterior samples at all, indicating severe convergence issues. Seven out of these ten chains belonged to the population condition set $N = 100, \ \lambda_{c1,6} = 0.2$, two to condition set $N = 200, \ \lambda_{c1,6} = 0.2$ and one replication to $N = 100, \ \lambda_{c1,6} = 0.5$. There was no set of conditions for which, on average, the proportion of divergent transitions exceeded 5%. There were also no replications that did not satisfy our convergence criteria regarding the effecitve sample size  $N_{Eff}$  and $\hat{R}$.

### RHSP

The RHSP showed weaker performance in terms of convergence than the
SVNP in general. In total 742 replications were removed for not reaching
convergence.
...
main conclusions. We decided
to present the findings with all hyper-parameters set to one, as this is
a logical default hyper-parameter configuration under the scale of a
standardized CFA model. This is appropriate, as the results did not differ 
substantially between different hyper-parameter configurations[^7]. The
replications for this hyper-parameter configuration showed good
convergence, such that only a single replication had to be removed.

[^7]: See <https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/RHSP> for the results of the RHSP with different hyper-parameter settings.

```{r, dev='cairo_pdf', include=T, echo=F, warning=F, fig.cap="Mean Absolute Bias in the Model Parameters (N = 100). Per set of parameters that showed an identical pattern, the first parameter was used to represent all other parameters, e.g.,  cross loading 2 was plotted representative for cross loading 3-5. All hyper-parameters of the RHSP are set to 1 in the results presented here. ", fig.height=7}
# load plot
library(patchwork)
readr::read_rds("~/1vs2StepBayesianRegSEM/Rmd/figures/allAtOnce.RDS") 
...
patterns in the bias of the RHSP were almost identical to that of the
SVNP with $\sigma^2 = 0.1$. For $\lambda_{c1,6} = 0.5$ the SVNP with hyper-prior the bias exceeded those of the RHSP and all other configurations of the SVNP. Note that for both priors the bias in these
...
$\sigma^2 = 0.1$ and to the SVNP with hyper-prior.
...
e.g., $\bar{\lambda}_{m 3} = 1.15$. The RHSP, the SVNP with
$\sigma^2 = 0.1$ and SVNP with hyper-prior had low levels of bias for the main loadings both with
...
shrunken to zero. For the SVNP with $\sigma^2 = 0.1$, the SVNP with hyper-prior and the RHSP,
...
```{r, dev='cairo_pdf', include = T, warning = F, fig.cap='Mean Power and Type-I-Error Rates in Selecting non-zero Cross Loadings (N = 100). All hyper-parameters of the RHSP are set to 1 in the results presented here. The red dashed horizontal lines represents a Power level of 0.80.', fig.height=10, fig.width=8}
library(patchwork)
library(ggplot2)
...
# combine power plots
powerAllAtOnce <- powerSVNP + powerRHSP + typeISVNP   + typeIRHSP 
powerAllAtOnce + plot_layout(guides = "collect", ncol = 2) & theme(legend.position = "bottom", legend.title = element_blank())
```
...
representative for the two truly non-zero cross loadings). Moreover, results of the RHSP are again presented with all hyper-parameters set to
one, and we focus on the results for N = 100[^8]. In the plots summarizing the Power the horizontal red dashed line
indicates the minimum power of .80 recommended by @muthen_bayesian_2012.
According to @cham_estimating_2012, the maximally acceptable
...
posterior means will never be entirely zero [@zhang_criteria_2021]. The
...
[^8]: The results for N = 200 of the RSHP, and for the other hyperparmatersettings, can be found on <https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/RHSP>. The results for the SVNP with n = 200 can be found on <https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/SVNP>. The results for the SVNP with hyper-prior and n = 200 can be found on <https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/analyses/SVNP_hyper>. 

For both priors and across most conditions, the Power fell under the
desired threshold of .80. For the SVNP with hyper-prior, under all selection rules, and for both $N = 100$ and $N = 200$ the Power fell under the treshold when the true size of the cross loading was 0.2. With cross loadings of 0.5, however, all selection rules except for the wider credible intervals (80%, 90% and 95%) did reach the desired Power. For the SVNP with $\sigma^2 = 0.001$ non-zero cross loadings were always over-shrunken so much that they were never selected as non-zero. Under $\sigma^2 = 0.01$, the situation improved
...
@muthen_bayesian_2012. The desired Power was exclusively reached in
...
and the RHSP exceeded the desired maximum. The SVNP with hyper-prior reached the desired threshold with all section rules except for the three widest credible intervals (80%, 90% and 95%), both for $N = 100$ and $N = 200$. For the SVNP, with
...
intervals lead to an undesirablelarge Type-I error rate. With
...
This was the first study to apply the Regularized Horseshoe Prior [RHSP,
@piironen_sparsity_2017] to select cross loadings in CFA. A comparison
to the Bayesian CFA approach by @muthen_bayesian_2012 was made, where
...
solution. However, under some hyper-parameter settings the performance of
...
loadings itself and, consequently, the other model parameters. Both the SVNP with hyper-prior and the RHSP performed very similar to the SVNP with $\sigma^2 = 0.1$.

The SVNP with a fixed variance of $\sigma^2 = 0.1$, the SVNP with hyper-prior and the RHSP were all able to estimate the model parameters with little bias. The RHSP did not outperform the SVNP with hyper-prior and showed substantially poorer convergence. Hence, for the simple CFA model in this study, the SVNP with hyper-prior struck the most ideal balance in model simplicity and model complexity by (1) yielding accurate solutions; (2) not sacrificing reliable estimation (3) not requiring a sensible a priori choice about the right amount of shrinkage (in contrast to the SVNP with fixed variance). In more complex models the refined nature of the RHSP with its several hyper-priors may ultimately prove advantageous, even over the SVNP with hyper-prior, as it allows for even more flexibility. Future research may compare the SVNP to the RHSP in the regularization of more complex SEM models.

Under simple models such as the one employed in this study
using the least complex version of SVNP, with the variance fixed to a relatively large value of $\sigma^2$ may also be advisable. This approach performed as well as both hyper-prior approaches. It also remains the most accessible approach, due to it's simpleness and availability across Bayesian CFA software. However, simply using larger values of $\sigma^2$ with the SVNP is no general solution. In practice, models may include more structural
...
to over-fitting. Because of this, and based on our results, we advise against the use of SVNP with fixed variance and instead recommend a SVNP with a hyperprior or the RHSP. 
...
LASSO, Spike-and-Slab Prior, @lu_bayesian_2016; @zhang_criteria_2021]
generally need much larger sample sizes than the ones employed in this
study to reach desirable levels of Power and Type-I-Error rates [see
@jacobucci_regularized_2016; @lu_bayesian_2016, who show that
...
@zhang_bayesian_2024 found the RHSP to perform well in selecting
...
still works as well as the SVNP with hyper-prior and substantially better than the SVNP with fixed variances, even with small
...
such as projection predictive inference
[@piironen_sparsity_2017] may be more successful in making a selection
that meets the desired balance in Power and Type-I-Error rate.

The large levels of bias found for the majority of fixed-variance configurations of the SVNP are not surprising, given the original approach
explicitly asking for a second step to circumvent bias
[@muthen_bayesian_2012]. However, the fact that the SVNP performed so
...
[@muthen_bayesian_2012]. An important next step is thus to assess the
...
practice [@zhang_criteria_2021]. It is also desirable to assess the
performance of the RHSP in regularizing model parameters in more complex
SEM models. For instance, extending the work by @zhang_bayesian_2024 who
...
adding another latent variable and with it another cross loading to their model
...
regularization of other structural parameters, such as indirect effects in
...
building on the current study. Regarding the SVNP, incorporating more hyper-prior configurations of the SVNP [see
for instance @lu_bayesian_2016] may prove valuable in allowing for a
...
@van_erp_bayesian_2023 emphasizes the need for user-friendly software for
...
[e.g., by adding it to the `BLAVAAN` package, @merkle_efficient_2020].
Only then applied researchers can actually use it in practice. 

Despite the limitations named, the current study forms a valuable
contribution to the current literature. We illustrate limitations of the currently available SVNP approach with fixed variances and investigate two alternatives: the RHSP and a SVNP with hyperprior, both of which are better able at avoiding substantial bias. However, more research is needed to improve the variable selection properties of shrinkage priors in Bayesian regularized SEM.
...
Data and materials are available at the github repository that is cited throughout the text: https://github.com/JMBKoch/1vs2StepBayesianRegSEM/.

## Code availability (software application or custom code)

Data and materials are available at the github repository that is cited throughout the
text: https://github.com/JMBKoch/1vs2StepBayesianRegSEM/
