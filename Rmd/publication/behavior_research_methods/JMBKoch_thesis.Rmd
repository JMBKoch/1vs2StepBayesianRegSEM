...
regression [@piironen_sparsity_2017; @van_erp_shrinkage_2019]. However,
...
@hans_bayesian_2009; see @van_erp_shrinkage_2019 for the Bayesian
...
### The SVNP with a hyper-prior 

The classical Bayesian CFA approach by @muthen_bayesian_2012 asks for a fixed variance parameter. However, it is common in  regularization to set hyper-priors on the parameters of priors, in order to allow for more flexibility in model estimation [@van_erp_shrinkage_2019] Such approach can lead to more sparsimonious models, as it allows for a more flexible model estimation, where not all parameters need to be shrunken in the same (heavy) amount. Software packages for Bayesian CFA such as Mplus [@muthen_mplus_1998] and BLAVAAN [@merkle_efficient_2020] have for the longest time not allowed to set hyper-priors for the variance of the SVNP. Only in probabilistic Bayesian modeling software such as `Stan` this possibility has been available [@stan_development_team_stan_2021; @van_erp_bayesian_2023]. Since such software packages may not be a practical choice for applied researchers in terms of user-friendliness, the fixed-variance approaches has remained a relevant go-to approach [@van_erp_bayesian_2023]. Recently, the BLAVAAN package has been updated to allow setting hyper-priors for the variance parameter of the SVNP. (*Als ik het goed begreep kan je daar dus nu wel een hyper-prior bepalen? REF naar blavaan versie release notes o.i.d? Ik probeeder i.i.g. om duidelijk te maken dat de fixed-variance approach door z'n availability vergeleken met de hyper-prior approach nog steeds harstikke relevant is om te onderzoeken.*). 

*Hier ben ik niet zeker wat handig is om op te schrijven...Lijkt me een goed idee om dit meer stapsgewijs op te bouwen*

In it's most simple form, one can set a half-Cauchy prior centered around 0 and with a scale of 1 on the standard deviation $\sigma$ of the SVNP. In summary, the SVNP for a cross-loading $\lambda_{c,jk} \lambda_{c,jk}$ is then specified as:
...
```{r, cache=T, dev='cairo_pdf', include=T, echo=F, warning=F, fig.cap="Density Plots of the Shrinkage Priors of Interest. A shiny app where the priors can be plotted for all hyperparameters settings in this study can be found on <https://jmb-koch.shinyapps.io/shiny/>."}
# Make Figure 1 with 
...
data.frame(dens = c(smallVar, regHs), 
          prior = as.factor(rep(c("SVNP (\u03c3\u00B2 = 0.01)", "RHSP (all  hyperparameters set to 1)"), each = ndraws)),
          asymp = rep(0, ndraws)) %>% 
...
[^4]: Note that a hyper-prior was set for $\sigma$ and not $\sigma^2$ as this is the way to specify a normal distribution Stan. We chose to refer to $\sigma^2$ in the general introduction of this shrinkage prior as it aligns more closely with the classical introduction of this approach by @muthen_bayesian_2012.
...
With the hyper-prior, the SVNP showed weaker but still acceptable convergence. There were ten replications yielded no posterior samples at all, indicating sever convergence issues. Seven out of these ten chains belonged to the condition set $N = 100, \ \lambda_{c1,6} = 0.2$, two $N = 200, \ \lambda_{c1,6} = 0.2$ and one chain $N = 100, \ \lambda_{c1,6} = 0.5$. There was no set of conditions for which, on average, the proportion of divergent transitions exceeded 5%. There were no replications that did not satisfy our convergence criteria regarding the effecitve sample size  $N_{Eff}$  and $\hat{R}$.
...
did not differ between different hyperparameter configurations[^7]. The
...
e.g., $\bar{\lambda}_{m 3} = 1.15$. Both the RHSP, the SVNP with
...
```{r, dev='cairo_pdf', include = T, warning = F, fig.cap='Mean Power and Type-I-Error Rates in Selecting non-zero Crossloadings (N = 100). All hyperparameters of the RHSP are set to 1 in the results presented here. The red dashed horizontal lines represents a power level of 0.80.', fig.height=10, fig.width=8}
library(patchwork)
library(ggplot2)
...
While the SVNP with a sufficiently large fixed variance estimated the simple CFA
model of this study with little bias, only the two priors with a hyper-prior were able to do so consistently. This illustrate the key advantage of using a hyper-prior approach over using fixed shrinkage parameters. The RHSP did not outperform the SVNP with a hyper-prior and showed substantially poorer convergence. Hence, for the simple CFA model in this study, the SVNP with hyper-prior struk the most ideal balance in model simplicity and model complexity, by yielding accurate solutions without sacrificing reliable estimation. In more complex models the refined nature of the RHSP with its several hyperpriors may ultimately prove advantegous even over the SVNP with hyper-prior, as it allows for even more flexibility. Future research may compare the SVNP to the RHSP in regularization of more complex SEM models. It is advised to include more refined hyper-prior configurations for the SVNP (REF needed) in such research to allow for a fair comparison. The current results do suggest, however, that for simple CFA model, using a RHSP is an overkill solution that may lead to more problems in terms of actually being able to estimate the model than benefits in terms of accurate shrinkage.  

The convergence issues of the RHSP followed a clear pattern, with issues arising with large cross loadings, small sample sizes and small scales for the local and global shrinkage parameter. While this may suggest that one should simply avoid using small values for the scales, using larger scales that shrink parameters less may lead to identification problems in more complex SEM models. Future research is
...
may also incorporate different configurations of `Stan's
No-U-Turn-Sampler, as well as larger burnin-periods, which help to
overcome convergence issues partially. Next to this, the RHSP may thus work more reliably with larger sample sizes. In sum, there remain thus plenty of potential use-cases for the RHSP over the SVNP with hyper-prior in regularizing more complex models on larger datasets. In the absence of user-friendly
...
approach for applied researchers. Especially since the SVNP can be used with a hyperprior in BLAVAAN nowadays (REF), using the RHSP may only be worth the effort for applied research in specific edge cases, where model complexity and data-availability make the RHSP the most effecient approach.
...
to over-fitting. Because of this, the SVNP with hyper-prior or the RHSP might prove as more advisable for more complex models. In particular the RHSP with its refined nature in giving prior mass only where strictly
