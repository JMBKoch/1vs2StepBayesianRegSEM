...
@van_der_pas_horseshoe_2014; @yamaguchi2023fully]. However, due to its
...
We included a configuration of the SVNP where a half-Cauchy prior centered around 0 and with a scale of 1 was set as hyperprior for the standard-deviation $\sigma$[^4]. (not the variance $\sigma^2$). Given the relatively simple nature of the model and the parameters to shrink, we chose to only include one hyper-parameter configuration of this prior in the study. We deemed this to be sufficient to assess potential benefits of a hyperprior approach over using a fixed variance, given the simple nature of the model studied. 
