...
### The SVNP with a hyperprior 

The classical Bayesian CFA approach by @muthen_bayesian_2012 asks for a fixed variance parameter. However, it is common in  regularization to set hyperpriors on the parameters of priors, in order to allow for more flexibility in model estimation [@van_erp_shrinkage_2019] Such approach can lead to more sparsimonious models, as it allows for a more flexible model estimation, where not all parameters need to be shrunken in the same (heavy) amount. Software packages for Bayesian CFA such as Mplus [@muthen_mplus_1998] and BLAVAAN [@merkle_efficient_2020] have for the longest time not allowed to set hyperpriors for the variance of the SVNP. Only in probabilistic Bayesian modeling software such as Stan this possibility has been available [@stan_development_team_stan_2021; @van_erp_bayesian_2023]. Since such software packages may not be a practical choice for applied researchers in terms of user-friendliness, the fixed-variance approaches has remained a relevant go-to approach [@van_erp_bayesian_2023]. Recently, the BLAVAAN package has been updated to allow setting hyperpriors for the variance parameter of the SVNP. (*Als ik het goed begreep kan je daar dus nu wel een hyperprior bepalen? REF naar blavaan versie release notes o.i.d? Ik probeeder i.i.g. om duidelijk te maken dat de fixed-variance approach door z'n availability vergeleken met de hyperprior approach nog steeds harstikke relevant is om te onderzoeken.*). 
...
We included a configuration of the SVNP where a half-Cauchy prior centered around 0 and with a scale of 1 was set as hyperprior for the standard-deviation $\sigma$ (not the variance $\sigma^2$)[^4]. We deemed this relatively simple specification to be suitable for assessing the benefits of a hyperprior over a fixed variance, given the simple nature of the model studied. 
...
### SVNP with hyperprior

With the hyperprior, the SVNP showed weaker but still acceptable convergence. There were ten replications yielded no posterior samples at all, indicating sever convergence issues. Seven out of these ten chains belonged to the condition set $N = 100, \ \lambda_{c1,6} = 0.2$, two $N = 200, \ \lambda_{c1,6} = 0.2$ and one chain $N = 100, \ \lambda_{c1,6} = 0.5$. There was no set of conditions for which, on average, the proportion of divergent transitions exceeded 5%. There were no replications that did not satisfy our convergence criteria regarding the effecitve sample size  $N_{Eff}$  and $\hat{R}$.
...
$\sigma^2 = 0.1$ and to the SVNP with hyperprior.
...
$\sigma^2 = 0.1$ and SVNP with hyperprior had low levels of bias for the main loadings both with
...
shrunken to zero. For the SVNP with $\sigma^2 = 0.1$, the SVNP with hyperprior and the RHSP,
...
and the RHSP exceeded the desired maximum. The SVNP with hyperprior reached the desired threshold with all section rules except for the three widest credible intervals (80%, 90% and 95%), both for $N = 100$ and $N = 200$. For the SVNP, with
