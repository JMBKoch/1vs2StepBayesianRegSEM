# presamples from the priors under all hyperparameter conditions such that the app runs smoothly# Load necessary libraries
library(LaplacesDemon)
library(TeachingDemos)

# Parameters
ndraws <- 1e+05# Sample size
...
svnp_samples <- lapply(sigma_squ, function(sigma) {
  rnorm(ndraws, mean = 0, sd = sqrt(sigma))
})
